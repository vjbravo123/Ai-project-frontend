...
export default function NewsAgentPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="p-6 rounded-2xl border border-slate-800 bg-[#0c1222] space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full py-2.5 px-3.5 rounded-lg bg-slate-900 border border-slate-800 text-sm text-slate-100 focus:outline-none focus:border-cyan-500"
            />
          </div>
          <div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button
            ...
            disabled={isLoading}
            className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-white font-semibold text-sm flex items-center gap-2 disabled:opacity-40 shadow-lg shadow-cyan-500/25"
          >
      ...
      {currentRun && (
        <div className="rounded-2xl border border-slate-800 bg-[#0c1222] p-6 space-y-4">
          ...
          {activeTab === 'preview' ? (
            <div
              className="p-6 rounded-xl bg-white text-slate-900 overflow-x-auto shadow-inner min-h-[300px] prose max-w-none"
              dangerouslySetInnerHTML={{ __html: currentRun.html }}
            />
          ) : (
            <pre className="p-4 rounded-xl bg-slate-950 font-mono text-xs text-cyan-300 overflow-x-auto border border-slate-800 leading-relaxed whitespace-pre-wrap">
              {currentRun.markdown}
            </pre>
          )}
        </div>
